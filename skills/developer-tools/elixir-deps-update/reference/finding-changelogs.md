...
# Without gh (Codeberg-hosted projects often have only tea installed) —
# unauthenticated, so subject to GitHub's 60-requests/hour anonymous rate limit
curl -s "https://api.github.com/repos/<owner>/<repo>/releases" \
  | jq '.[] | {tag: .tag_name, body: .body}'
```

Both hit GitHub regardless of where *this* project is hosted — the package's
own repo is what's being read, and nearly every Hex package lives on GitHub.
`tea` is not a substitute here; it talks to Gitea instances only.
...
references are relative to *that* repo. Pasted verbatim into your PR body, the
forge re-resolves them against **yours**. GitHub and the Gitea family (Codeberg,
Gitea, Forgejo) both do this, and both do it the same way:
...
- `@handle` notifies a real account **on the forge you are posting to**, with no
  connection to your repo. On Codeberg that is a Codeberg user who happens to
  share the upstream GitHub contributor's handle.
- A bare commit SHA autolinks and resolves to nothing.

Strip `@handle` credits outright — a thanks-to line tells the reader of a
dependency bump nothing. Then rewrite each issue reference into the form the
**target forge** resolves correctly. Step 3 above yields the package's repo if
you don't already have it.

### Rewriting issue references per forge

The qualified `<owner>/<repo>#123` shorthand is **instance-local**: each forge
resolves it against its own host. That makes it correct on GitHub for a package
hosted on GitHub, and wrong everywhere else.

| PR opened on | Upstream package repo | Write the reference as |
| --- | --- | --- |
| GitHub | GitHub (the normal case) | `<owner>/<repo>#123` |
| Codeberg / Gitea / Forgejo | GitHub (the normal case) | `https://github.com/<owner>/<repo>/issues/123` |
| Codeberg / Gitea / Forgejo | same instance | `<owner>/<repo>#123` |

A full URL is the only form that is safe on every forge, so prefer it whenever
in doubt — it renders as a plain link and never silently repoints. If the
package's repo can't be established at all, drop the reference; the sentence
